    ...
    if (token) {
        ...
        jwt.verify(token, 'super secret code', (err, decodedToken) => {
            if (err || (decodedToken.authType !== 'admin' && decodedToken.authType !== 'instructor' && decodedToken.authType !== 'student')) {
                ...
                // Preserving admin and instructor authType, setting local variables for email, first name, last name, scheduled courses, and proceeding to the next middleware or route
